...
SafetensorsLayer_parse (json_t *layer, SafetensorsLayer *h)
{
  ...
  if (!json_is_string (dtype) || !json_is_array (shape)
      || !json_is_array (data_offset))
    {
...
Safetensors_new (const char *file_path)
{
  ...
  h->json_root = NULL;
  CHECK_STATUS_PANIC (Safetensors_parse (h, header_content),
...
Safetensors_print (Safetensors *h)
{
  ...
  json_object_foreach (h->json_root, key, value)
  {
    ...
    SafetensorsLayer *layer
        = (SafetensorsLayer *)malloc (sizeof (SafetensorsLayer));
    CHECK_MALLOC_PANIC (layer, key);
    CHECK_STATUS (SafetensorsLayer_parse (json_layer, layer),
                  "Error parsing tensor header %s\n", key)
...
Safetensors_load_matrix (const char *tensor_name, const Safetensors *header)
{
  // Get layer metadata
  json_t *json_layer
      = GET_JSON_OBJECT_PANIC (header->json_root, tensor_name, json_layer);
  SafetensorsLayer *layer
      = (SafetensorsLayer *)malloc (sizeof (SafetensorsLayer));
  CHECK_MALLOC_PANIC (layer, tensor_name);
  CHECK_STATUS_PANIC (SafetensorsLayer_parse (json_layer, layer),
                      "Error parsing tensor header %s\n", tensor_name)
  ...
  int start_index
      = HEADER_SIZE_PART_SIZE + header->header_size + layer->data_offset[0];
  ...
  if (layer->dtype == F32)
    {
      ...
      memcpy (buff, (char *)header->map + start_index,
              nb_elements * sizeof (float));
...
CallmStatusCode
Safetensors_get_layer_by_name (const Safetensors *h, const char *layer_name,
                               SafetensorsLayer *layer)
{
