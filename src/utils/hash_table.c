...
unsigned int
HashTable_hash (const char *key, int table_size)
{
  unsigned int hash = 0;
  while (*key)
    {
...
HashTableNode *
HashTableNode_new (const char *key, const void *value, size_t value_size)
{
...
HashTable_new (int size)
{
  ...
  hash_table->table
      = (HashTableNode **)malloc (sizeof (HashTableNode *) * size);
...
CallmStatusCode
HashTable_resize (HashTable *hash_table)
{
  int new_size = hash_table->size * 2;
  HashTableNode **new_table
      = (HashTableNode **)malloc (sizeof (HashTableNode *) * new_size);
...
CallmStatusCode
HashTable_insert (HashTable *hash_table, const char *key, const void *value,
                  size_t value_size)
{
...
void *
HashTable_get (HashTable *hash_table, const char *key)
{
  unsigned int index = HashTable_hash (key, hash_table->size);
  HashTableNode *temp = hash_table->table[index];
  while (temp)
...
CallmStatusCode
HashTable_delete (HashTable *hash_table, const char *key)
{
  unsigned int index = HashTable_hash (key, hash_table->size);
  HashTableNode *temp = hash_table->table[index];
...
char **
HashTable_keys (HashTable *hash_table)
{
