...
LinearMap_resize (LinearMap *map)
{
  ...
  LinearMapItem *new_items
      = (LinearMapItem *)malloc (sizeof (LinearMapItem) * new_capacity);
...
LinearMap_new ()
{
  ...
  (*map).items = (LinearMapItem *)malloc (sizeof (LinearMapItem)
                                          * LINEAR_MAP_DEFAULT_CAPACITY);
  return map;
}
...
LinearMap_insert (LinearMap *map, char *key, void *value, int value_size)
{
  ...
  if (item == NULL)
    {
      LOG_ERROR (
          "Failed to insert item into linear map: failed to create new item");
      return ERROR;
    }
